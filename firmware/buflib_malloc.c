 ...
 *
 * The primary objective of this backend is
 * a) avoiding the shrink callback for the audio buffer which currently
 *    involves stoping playback
 * b) enable to detect memory leaks and use-after-free by mapping buflib
 *    allocations to host's malloc() which enables this kind of debugging
 *
 * In order to make (a) possible there is no possibility for new allocations
 * to affect existing ones: Neither move_callback() nor shrink_callback()
 * will be run. Instead, there is no limit on allocations.
 *
 * In contrast to buflib.c, there is not a single array of "union buflib_data"
 * elements. Instead, each buflib allocation maps to a malloc'd one, which
 * do not relate to each other. However, the per-alloc-cookie format is
 * maintained, as well as the handle table approach to access allocs via
 * handle index. Both allow similar code and debug output.
 */
...
struct buflib_context_extra {
    union buflib_data *buffer_ptr;
    size_t             buffer_pos;
    size_t             max_buffer_len;
};

static inline
struct buflib_context_extra *get_extra(struct buflib_context *ctx)
{
    return (struct buflib_context_extra *) ctx->buf_start;
}
...
{
    struct buflib_context_extra *extra;
    (void)buf;
    memset(ctx, 0, sizeof(*ctx));

    /* use buf_start to store pointer to extra data */
    extra = malloc(sizeof(struct buflib_context_extra));
    ctx->buf_start = (union buflib_data *)extra;

    extra->buffer_ptr = NULL;
    extra->buffer_pos = 0;
    extra->max_buffer_len = size / sizeof(union buflib_data);
...
{
    size_t avail, siz = B_ALIGN_UP(*size);
    struct buflib_context_extra *data = get_extra(ctx);
    siz /= sizeof(union buflib_data);
    avail = data->max_buffer_len - data->buffer_pos;

    if (!data->buffer_ptr)
    {   /* first call: allocate buffer */
        data->buffer_ptr = buflib_data_new(data->max_buffer_len);
    }

    /* calculate the buffer postion before substracting siz */;
    siz = siz ? MIN(siz, avail) : avail;
    data->buffer_pos += siz;
    *size = siz * sizeof(union buflib_data);

    return &data->buffer_ptr[data->buffer_pos];
}
...
buflib_buffer_in(struct buflib_context *ctx, int size)
{
    ...
    new_pos = data->buffer_pos - size;
    if (!new_pos || new_pos > data->buffer_pos) /* check for underrun */
    {   /* all of the buffer was returned back, time to free.
         * The next call to buflib_buffer_out() will create a fresh buffer */
...
{
    intptr_t rem = get_extra(ctx)->max_buffer_len;
    rem -= 16; /* space for future handles */
    rem *= sizeof(union buflib_data); /* make it bytes */
    rem -= 16; /* reserve 16 for the name */

    return rem;
}

/* this backend is always pefectly compacted (by definition), therefore
 * buflib_available() and buflib_allocatable() return the same */
size_t
buflib_allocatable(struct buflib_context* ctx)
{
    return buflib_available(ctx);
}
...
buflib_shrink(struct buflib_context* ctx, int handle, void** new_start, size_t new_size)
{
    ...
    else
    {
        ...
        /* new_handle is not needed anymore, we're keeping the old handle */
